...
struct book *add_book_to_library(struct book *);
...
void main_menu(){
    ...
    do{
        ...
        printf("\n\t\t\t\t     5.ADD BOOK TO LIBRARY "); // Add this line
        printf("\n\t\t\t\t     6.EXIT\n "); // Updated option numbers
        printf("\n\t\t\t*************************************************\n");
        printf("\n\t\t\t\t      Enter your choice: ");
        scanf("%d",&choice);
        switch(choice){
            ...
            case 4:{
                display_available_books(start_lib);
                break;
            }
            case 5:{
                add_book_to_library(start_lib); // Add this case
                break;
            }
            case 6:{
                exit(1);
    ...
    }while(choice!=6); // Update the condition to exit on option 6
}
...
struct book *add_book_to_library(struct book *start_lib) {
...
struct student *book_issue(struct student *start){
    ...
    if(start_lib==NULL){
        ...
        if(flag==1){
            ...
            printf("\n\t Enter your Registration Number: "); // Add this line
...
struct student *book_return(struct student *start){
    ...
    if(flag==1){
        ...
        if( d==1 ){
            ...
            printf("\n\t\t Registration Number: %d", ptr->reg_number);
...
void display_available_books(struct book *start_lib) {
    struct book *ptr;
    int i = 1;
